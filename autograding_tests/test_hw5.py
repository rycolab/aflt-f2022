...
def test_minimization_example_1_Real():
    fsa = FSA(Real)
    fsa.add_arc(State("c"), '1', State("e"), Real(0.087))
    fsa.add_F(State("c"), Real(0.068))
    fsa.add_arc(State("a"), '1', State("b"), Real(0.035))
    fsa.add_arc(State("a"), '2', State("c"), Real(0.079))
    fsa.set_I(State("a"), Real(0.132))
    fsa.add_arc(State("b"), '2', State("d"), Real(0.148))
    fsa.add_arc(State("b"), '1', State("a"), Real(0.091))
    fsa.add_arc(State("e"), '1', State("e"), Real(0.103))
    fsa.add_F(State("e"), Real(0.09))
    fsa.add_arc(State("d"), '1', State("e"), Real(0.136))
    fsa.add_F(State("d"), Real(0.067))

    mfsa = fsa.minimize()

    MFSA = FSA(Real)
    MFSA.add_arc(MinimizeState([State("b"), State("a")]), '1', MinimizeState([State("b"), State("a")]), Real(0.126))
    MFSA.add_arc(MinimizeState([State("b"), State("a")]), '2', MinimizeState([State("d"), State("e"), State("c")]), Real(0.227))
    MFSA.set_I(MinimizeState([State("b"), State("a")]), Real(0.132))
    MFSA.add_arc(MinimizeState([State("d"), State("e"), State("c")]), '1', MinimizeState([State("d"), State("e"), State("c")]), Real(0.326))
    MFSA.add_F(MinimizeState([State("d"), State("e"), State("c")]), Real(0.225))
...
def test_minimization_example_2_Real():
    fsa = FSA(Real)
    fsa.add_arc(State(1), 'a', State(2), Real(0.094))
    fsa.add_arc(State(1), 'b', State(3), Real(0.061))
    fsa.add_arc(State(1), 'c', State(4), Real(0.053))
    fsa.set_I(State(1), Real(0.068))
    fsa.add_arc(State(2), 'a', State(5), Real(0.037))
    fsa.add_arc(State(2), 'b', State(6), Real(0.087))
    fsa.add_arc(State(3), 'a', State(10), Real(0.064))
    fsa.add_arc(State(3), 'b', State(7), Real(0.112))
    fsa.add_arc(State(4), 'a', State(8), Real(0.06))
    fsa.add_arc(State(4), 'b', State(9), Real(0.148))
    fsa.add_arc(State(5), 'a', State(15), Real(0.141))
    fsa.add_arc(State(5), 'b', State(10), Real(0.084))
    fsa.add_arc(State(6), 'a', State(10), Real(0.096))
    fsa.add_arc(State(6), 'b', State(11), Real(0.155))
    fsa.add_arc(State(7), 'a', State(10), Real(0.121))
    fsa.add_arc(State(7), 'b', State(11), Real(0.068))
    fsa.add_arc(State(8), 'a', State(12), Real(0.096))
    fsa.add_arc(State(9), 'a', State(12), Real(0.054))
    fsa.add_arc(State(9), 'b', State(15), Real(0.134))
    fsa.add_arc(State(10), 'a', State(13), Real(0.127))
    fsa.add_arc(State(10), 'b', State(15), Real(0.037))
    fsa.add_arc(State(11), 'a', State(13), Real(0.102))
    fsa.add_arc(State(12), 'a', State(14), Real(0.135))
    fsa.add_arc(State(12), 'c', State(15), Real(0.141))
    fsa.add_arc(State(13), 'b', State(15), Real(0.116))
    fsa.add_arc(State(14), 'd', State(15), Real(0.128))
    fsa.add_F(State(15), Real(0.049))


    MFSA = FSA(Real)
    MFSA.add_arc(MinimizeState([State(2)]), 'a', MinimizeState([State(5)]), Real(0.037))
    MFSA.add_arc(MinimizeState([State(2)]), 'b', MinimizeState([State(6), State(7)]), Real(0.087))
    MFSA.add_arc(MinimizeState([State(11)]), 'a', MinimizeState([State(13)]), Real(0.102))
    MFSA.add_arc(MinimizeState([State(14)]), 'd', MinimizeState([State(15)]), Real(0.128))
    MFSA.add_arc(MinimizeState([State(9)]), 'a', MinimizeState([State(12)]), Real(0.054))
    MFSA.add_arc(MinimizeState([State(9)]), 'b', MinimizeState([State(15)]), Real(0.134))
    MFSA.add_arc(MinimizeState([State(6), State(7)]), 'a', MinimizeState([State(10)]), Real(0.217))
    MFSA.add_arc(MinimizeState([State(6), State(7)]), 'b', MinimizeState([State(11)]), Real(0.223))
    MFSA.add_arc(MinimizeState([State(8)]), 'a', MinimizeState([State(12)]), Real(0.096))
    MFSA.add_arc(MinimizeState([State(3)]), 'a', MinimizeState([State(10)]), Real(0.064))
    MFSA.add_arc(MinimizeState([State(3)]), 'b', MinimizeState([State(6), State(7)]), Real(0.112))
    MFSA.add_arc(MinimizeState([State(10)]), 'a', MinimizeState([State(13)]), Real(0.127))
    MFSA.add_arc(MinimizeState([State(10)]), 'b', MinimizeState([State(15)]), Real(0.037))
    MFSA.add_arc(MinimizeState([State(1)]), 'a', MinimizeState([State(2)]), Real(0.094))
    MFSA.add_arc(MinimizeState([State(1)]), 'b', MinimizeState([State(3)]), Real(0.061))
    MFSA.add_arc(MinimizeState([State(1)]), 'c', MinimizeState([State(4)]), Real(0.053))
    MFSA.set_I(MinimizeState([State(1)]), Real(0.068))
    MFSA.add_arc(MinimizeState([State(13)]), 'b', MinimizeState([State(15)]), Real(0.116))
    MFSA.add_arc(MinimizeState([State(5)]), 'a', MinimizeState([State(15)]), Real(0.141))
    MFSA.add_arc(MinimizeState([State(5)]), 'b', MinimizeState([State(10)]), Real(0.084))
    MFSA.add_F(MinimizeState([State(15)]), Real(0.049))
    MFSA.add_arc(MinimizeState([State(12)]), 'a', MinimizeState([State(14)]), Real(0.135))
    MFSA.add_arc(MinimizeState([State(12)]), 'c', MinimizeState([State(15)]), Real(0.141))
    MFSA.add_arc(MinimizeState([State(4)]), 'a', MinimizeState([State(8)]), Real(0.06))
    MFSA.add_arc(MinimizeState([State(4)]), 'b', MinimizeState([State(9)]), Real(0.148))
...
def test_example_minimize_3_Real():
    
    fsa = FSA(Real)
    fsa.add_arc(State(1), 'a', State(2), Real(0.039))
    fsa.add_arc(State(1), 'b', State(3), Real(0.102))
    fsa.add_arc(State(1), 'c', State(4), Real(0.116))
    fsa.set_I(State(1), Real(0.099))
    fsa.add_arc(State(2), 'a', State(5), Real(0.158))
    fsa.add_arc(State(2), 'b', State(6), Real(0.107))
    fsa.add_arc(State(3), 'a', State(10), Real(0.114))
    fsa.add_arc(State(3), 'b', State(7), Real(0.063))
    fsa.add_arc(State(4), 'a', State(8), Real(0.112))
    fsa.add_arc(State(4), 'b', State(9), Real(0.083))
    fsa.add_arc(State(5), 'b', State(10), Real(0.07))
    fsa.add_arc(State(6), 'b', State(11), Real(0.094))
    fsa.add_arc(State(7), 'a', State(10), Real(0.108))
    fsa.add_arc(State(7), 'b', State(11), Real(0.09))
    fsa.add_arc(State(8), 'a', State(12), Real(0.146))
    fsa.add_arc(State(9), 'a', State(12), Real(0.113))
    fsa.add_arc(State(10), 'a', State(13), Real(0.079))
    fsa.add_arc(State(10), 'b', State(15), Real(0.051))
    fsa.add_arc(State(11), 'a', State(13), Real(0.074))
    fsa.add_arc(State(11), 'b', State(15), Real(0.117))
    fsa.add_arc(State(12), 'a', State(14), Real(0.136))
    fsa.add_arc(State(12), 'c', State(15), Real(0.143))
    fsa.add_arc(State(13), 'b', State(15), Real(0.053))
    fsa.add_arc(State(14), 'd', State(15), Real(0.056))
    fsa.add_F(State(15), Real(0.159))

    MFSA = FSA(Real)
    MFSA.add_arc(MinimizeState([State(2)]), 'a', MinimizeState([State(5), State(6)]), Real(0.158))
    MFSA.add_arc(MinimizeState([State(2)]), 'b', MinimizeState([State(5), State(6)]), Real(0.107))
    MFSA.add_arc(MinimizeState([State(14)]), 'd', MinimizeState([State(15)]), Real(0.056))
    MFSA.add_arc(MinimizeState([State(8), State(9)]), 'a', MinimizeState([State(12)]), Real(0.259))
    MFSA.add_F(MinimizeState([State(15)]), Real(0.159))
    MFSA.add_arc(MinimizeState([State(7)]), 'a', MinimizeState([State(10), State(11)]), Real(0.108))
    MFSA.add_arc(MinimizeState([State(7)]), 'b', MinimizeState([State(10), State(11)]), Real(0.09))
    MFSA.add_arc(MinimizeState([State(3)]), 'a', MinimizeState([State(10), State(11)]), Real(0.114))
    MFSA.add_arc(MinimizeState([State(3)]), 'b', MinimizeState([State(7)]), Real(0.063))
    MFSA.add_arc(MinimizeState([State(1)]), 'a', MinimizeState([State(2)]), Real(0.039))
    MFSA.add_arc(MinimizeState([State(1)]), 'b', MinimizeState([State(3)]), Real(0.102))
    MFSA.add_arc(MinimizeState([State(1)]), 'c', MinimizeState([State(4)]), Real(0.116))
    MFSA.set_I(MinimizeState([State(1)]), Real(0.099))
    MFSA.add_arc(MinimizeState([State(5), State(6)]), 'b', MinimizeState([State(10), State(11)]), Real(0.164))
    MFSA.add_arc(MinimizeState([State(13)]), 'b', MinimizeState([State(15)]), Real(0.053))
    MFSA.add_arc(MinimizeState([State(10), State(11)]), 'a', MinimizeState([State(13)]), Real(0.153))
    MFSA.add_arc(MinimizeState([State(10), State(11)]), 'b', MinimizeState([State(15)]), Real(0.168))
    MFSA.add_arc(MinimizeState([State(12)]), 'a', MinimizeState([State(14)]), Real(0.136))
    MFSA.add_arc(MinimizeState([State(12)]), 'c', MinimizeState([State(15)]), Real(0.143))
    MFSA.add_arc(MinimizeState([State(4)]), 'a', MinimizeState([State(8), State(9)]), Real(0.112))
    MFSA.add_arc(MinimizeState([State(4)]), 'b', MinimizeState([State(8), State(9)]), Real(0.083))
